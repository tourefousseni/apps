...
def list(request, *args, **kwargs):
    list_gest    = Redevance_eau.objects.all().order_by('-id')
...
def detail(request, id):
    detail_view = Redevance_eau.objects.get(pk=id)
    context = {
        'gestion': detail_view,
    }
...
def update_mesure(request, id):
    mesure       = Redevance_eau.objects.get(id=id)
...
def update(request, id):
    update_gest = Redevance_eau.objects.get(pk=id)
...
def delete_mesure(request, id):
    gestion = Redevance_eau.objects.get(id=id)
    if request.method == 'POST':
        gestion.delete()
...
def report_carnet(request, id):
    detail_view = Redevance_eau.objects.get(id=id)
...
def report_all_mesure(request):
    page_obj = Redevance_eau.objects.all()
...
def report_mesure(request):
    page_obj = Redevance_eau.objects.all().order_by('id')
...
def carnet(request, id):
    mesure = Redevance_eau.objects.get(pk=id)
    # list_person = Person.objects.get(pk=id)
...
def search_mesure(request):
    search = request.GET.get('search')
    filter_data = Redevance_eau.objects.filter(Q(coude__icontains=search)|
